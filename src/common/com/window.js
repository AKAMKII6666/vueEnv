 ...
 */
import 'common/styles/window.css';

var _window = function (_config) {
    var self = this;
    this.screenState = "H";
    //判断是否为移动端
    self.isMobile = false;
    ...
    this.bindEvent = function () {
        ...
        this.title.mousedown(function () {
            ...
            mControl.addMouseMoveFunc('windowMove', function () {
                var p = {
                    ...
                    y: mControl.nowPosition.y - bP.y - $(window).scrollTop()
                }
        ...
        if (self.config.buttons.mode !== 'none' && self.config.buttons.mode !== 'free') {
    ...
    this.createButtons = function () {
        ...
        if (self.config.buttons.mode === 'free') {
            ...
            for (var i = 0; i < self.config.buttons.arr.length; i++) {
                ...
                var elem = $("<button class='b-w-b n-button'  >" + item.name + "</button>&nbsp;&nbsp;").appendTo(self.buttonContainer);
                if (i !== self.config.buttons.arr.length - 1) {
                    elem.css('margin-right', '50px');
                }
